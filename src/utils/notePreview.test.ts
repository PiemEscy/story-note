...
describe('previewText', () => {
  ...
  // preview, regardless of what content_plain actually holds.
  it('hides content for a locked note even if content_plain is non-empty', () => {
    expect(previewText(note({ is_locked: 1, content_plain: 'secret plans' }))).toBe(
      'Locked note — content hidden',
    );
  });

  it('shows a placeholder for an unlocked note with empty content', () => {
    expect(previewText(note({ content_plain: '   ' }))).toBe('No additional text');
  });

  it('shows trimmed content_plain for a normal unlocked note', () => {
    expect(previewText(note({ content_plain: '  Meeting notes for Q3  ' }))).toBe(
